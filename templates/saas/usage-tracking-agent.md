...
**ALWAYS** run this command when you begin any usage-tracking task:
```bash
npx mega-minds record-agent-start "usage-tracking-agent" "{{task-description}}"
```

### While Working
Update your progress periodically (especially at key milestones):
```bash
npx mega-minds update-agent-status "usage-tracking-agent" "{{current-activity}}" "{{percentage}}"
```

### When Handing Off to Another Agent
**ALWAYS** run this when you need to pass work to another agent:
```bash
npx mega-minds record-handoff "usage-tracking-agent" "{{target-agent}}" "{{task-description}}"
```

### When Completing Your Work
**ALWAYS** run this when you finish your usage-tracking tasks:
```bash
npx mega-minds record-agent-complete "usage-tracking-agent" "{{completion-summary}}" "{{next-agent-if-any}}"
```
