...
class StateMonitor {
        ...
        try {
            ...
            if (this.sessionManager && this.sessionManager.currentSession) {
                ...
                if (shouldAutoSave) {
                    ...
                    console.log(`💾 Session auto-saved with ${agentCount} active agents (significant change detected, throttled)`);
                } else {
                    console.log(`⏭️ Skipping auto-save for ${agentCount} agents (broadcast-triggered or minor change)`);
                }
            }
    ...
    async handleHandoffQueueChange(queueData) {
        try {
            ...
            if (this.sessionManager && this.sessionManager.currentSession) {
                ...
                await this.sessionManager.saveActiveSession(false); // Use throttling
                console.log(`💾 Session updated with handoff data (throttled)`);
            }
        ...
        try {
            ...
            if (this.sessionManager && this.sessionManager.currentSession) {
                ...
                if (overallProgress % 50 === 0 && overallProgress > 0) {
                    await this.sessionManager.saveActiveSession(false); // Use throttling
                    console.log(`💾 Session saved at ${overallProgress}% progress milestone (throttled)`);
                }
            }
    ...
    shouldTriggerAutoSave(activeAgents, agentCount) {
        ...
        if (shouldSave) {
            console.log(`🔍 Auto-save triggered: count=${hasCountChange}, agents=${hasAgentChange}, status=${hasStatusChange}`);
        }
