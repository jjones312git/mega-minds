...
class HookSetup {
    ...
    async run() {
        ...
        let answers;
        try {
            answers = await this.promptUserPreferences();
        } catch (error) {
            if (error.message.includes('force closed') || error.message.includes('null')) {
                console.log('\n❌ Interactive setup detected in non-interactive environment');
                console.log('');
                console.log('🖥️  This command needs to be run in your REGULAR TERMINAL, not Claude Code.');
                console.log('');
                console.log('📋 For non-technical users:');
                console.log('1. Open Terminal (Mac: Cmd+Space → "Terminal") or Command Prompt (Windows: Win+R → "cmd")');
                console.log('2. Navigate to your project: cd path/to/your/project');
                console.log('3. Run: npx mega-minds setup-hooks');
                console.log('');
                console.log('💡 Alternatively, see .claude/claude.md for detailed setup instructions');
                return;
            }
            throw error;
        }
        ...
        const questions = [
            {
                ...
                choices: [
                    {
                        name: 'Agent Coordination Tracking - Auto-detect Task tool usage for handoffs',
                        value: 'agentTracking',
                        checked: true
                    },
                    {
    ...
    generateHookConfiguration(answers) {
        ...
        // Initialize PostToolUse as an array to handle multiple configurations
        config.hooks.PostToolUse = [];

        if (answers.enabledHooks.includes('agentTracking')) {
